...
parser.add_argument('--modes', type=str, default='train validation test', help='choose train validation test')
parser.add_argument('--select_scenes', type=str, default=None, help='choose only certain scenes to extract 2D images')
parser.add_argument('--meta_data', default='scannet-preprocess/meta_data/', help='path to scannet meta data')
...
parser.add_argument('--label_map_file', default='/data/jokim/ScanNet/dataset/scannetv2-labels.combined.tsv',
...
def main():
    ...
    for mode in opt.modes.split():
        ...
        with open(opt.meta_data + f'scannetv2_{mode}.txt', 'r') as f:
            scenes_list = f.read().splitlines()
            choosen_scenes_list.extend(scenes_list)
            print(f"In {mode} {len(scenes_list)} are included.")
            
    print(f"Choosen {len(choosen_scenes_list)} scenes.")
    
    if opt.select_scenes is not None:
        tmp_scenes_list = []
        for sc in opt.select_scenes.split(','):            
            assert sc in choosen_scenes_list, "Selected scene is not in the scene list"
            tmp_scenes_list.append(sc)
        
        choosen_scenes_list = tmp_scenes_list
        print(f"Selected {choosen_scenes_list}.")
    
    scenes = choosen_scenes_list
    for i in range(0,len(scenes)):        
        path = os.path.join(opt.output_path, scenes[i])
