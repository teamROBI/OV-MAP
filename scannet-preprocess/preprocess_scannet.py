...
def handle_process(scene_path, output_path, labels_pd, train_scenes, val_scenes, test_scenes, parse_normals=True):
    ...
    if scene_id in train_scenes:
        ...
        output_ply_file = os.path.join(output_path, 'train', f'{scene_id}.ply')
        split_name = 'train'
    elif scene_id in val_scenes:
        output_file = os.path.join(output_path, 'val', f'{scene_id}.pth')
        output_ply_file = os.path.join(output_path, 'val', f'{scene_id}.ply')
        split_name = 'val'
    elif scene_id in test_scenes:
        output_file = os.path.join(output_path, 'test', f'{scene_id}.pth')
        output_ply_file = os.path.join(output_path, 'test', f'{scene_id}.ply')
        split_name = 'test'
    else:
        return
...
if __name__ == '__main__':
    ...
    # Check if the output_root already exists
    if os.path.exists(config.output_root):
        print(f"The output directory '{config.output_root}' already exists. Exiting the program.")
        sys.exit(1)  # Exit the program with a non-zero status to indicate an error

    # Load label map
    labels_pd = pd.read_csv('scannet-preprocess/meta_data/scannetv2-labels.combined.tsv', sep='\t', header=0)
    ...
    with open('scannet-preprocess/meta_data/scannetv2_test.txt') as test_file:
    ...
    _ = list(pool.map(handle_process, scene_paths, repeat(config.output_root), repeat(labels_pd), repeat(train_scenes),
                      repeat(val_scenes), repeat(test_scenes), repeat(config.parse_normals)))
