...
OVERLAP_CRITERION="iou3" #large_overlap(lo3), small overlap(so5), intersection over union(iou3)
VOXEL_SIZE=0.03
IMAGE_ITER=5
...
# HYPER_PARAMETERS="MC${MERGING_CRITERION}_OC${OVERLAP_CRITERION}_VX${VOXEL_SIZE}_IT${IMAGE_ITER}"  #ov_large_overlap_0.3, large_overlap_0.3, small_overlap_0.5, save_bivi, save
...
OUTPUT_FOLDER_DIRECTORY="$(pwd)/open_query/clip_output/${EXPERIMENT_NAME}/${MASK_MODEL}_${DEPTH_TYPE}/${HYPER_PARAMETERS}"
EVALUATION_OUTPUT_DIR="${OUTPUT_FOLDER_DIRECTORY}/CA_evaluation_result.txt"
