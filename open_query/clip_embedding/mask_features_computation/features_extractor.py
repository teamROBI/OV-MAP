...
class PointProjector:
    ...
    def get_visible_points_view(self):
        ...
        for i, idx in tqdm(enumerate(indices), total=len(indices)): # for each view
            ...
            sensor_depth = self.camera.load_depth(idx)
            ...
            if self.debug_vis_mode == 1:
                self.visualize_visible_points_view(idx, i, projected_points, visible_points_view)
            
        return visible_points_view, projected_points, resolution
    
    def visualize_visible_points_view(self, idx, i, projected_points, visible_points_view):
        img_pil = self.camera.images.load_image(idx)  # Load the image using your existing function
        ...
        visible_points = projected_points[i][visible_points_view[i]]
        ...
        # Display the result
        cv2.imshow(f"Visible Points in View", img)
        # cv2.imshow(f"Visible Points in View {idx}", img)
        
        # Wait for key press; if Esc is pressed, exit the loop
        key = cv2.waitKey(0)
            ...
            for j in range(masks.num_masks):
                ...
                if(len(proj_points) != 0):
                    ...
                    if self.debug_vis_mode == 2:
                        self.visualize_visible_points_in_view_in_mask(i, j, proj_points)
    ...
    def visualize_visible_points_in_view_in_mask(self, i, j, proj_points):
        img_pil = self.camera.images.load_image(self.indices[i])  # Load the image
        img = np.array(img_pil)  # Convert PIL to NumPy array
        img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)  # Convert to BGR for OpenCV
        
        for pt in proj_points:
            x, y = int(pt[0]), int(pt[1])
            if 0 <= x < img.shape[1] and 0 <= y < img.shape[0]:  # Ensure points are within bounds
                cv2.circle(img, (x, y), radius=2, color=(0, 0, 255), thickness=-1)  # Red dots
        
        # Display the visualization
        cv2.imshow(f"View {i}, Mask {j}", img)
...
class FeaturesExtractor:
    def __init__(self, 
        ...
        self.images = images
        self.device = device
        self.point_projector = PointProjector(camera, pointcloud, masks, vis_threshold, images.indices, debug_vis_mode)
    ...
    def extract_features(self, topk, multi_level_expansion_ratio, num_levels, num_random_rounds, num_selected_points, save_crops, out_folder,
                         save_subs, optimize_gpu_usage=False):
        if(save_crops):
        ...
        for mask in tqdm(range(num_masks)): # for each mask            
            ...
            for view_count, view in enumerate(topk_indices_per_mask[mask]): # for each view                
                ...
                if (point_coords.shape[0] > 0):
                    ...
                    if save_subs:
                        vis_info.append({"view":view, "image":self.images.images[view], "point_coords":point_coords, "bounding_boxes":bounding_boxes})
                        images_global.append(self.clip_preprocess(self.images.images[view]))
                    
                    ### DEBUG Visualization
                    if self.debug_vis_mode == 3:
                        self.visualize_bounding_boxes(self.images.images[view], bounding_boxes, levels, mask, view)
            ...
            if(len(images_crops) > 0):
                ...
                if save_subs:
                    with torch.no_grad():                    
                        image_global_input = torch.tensor(np.stack(images_global))
                        image_global_features = self.clip_model.encode_image(image_global_input.to(self.device)).float()
                        image_global_features /= image_global_features.norm(dim=-1, keepdim=True) #normalize
                    
                    masks_sub_list.append({"vis_info":vis_info, "crop_images_feature":image_features.cpu().numpy(), "global_images_feature":image_global_features.cpu().numpy()})
                    del images_global
                    
                # Free memory after processing
                del images_crops, image_input, image_features
                gc.collect()
        
        # for masks_sub in masks_sub_list:
        #     print(len(masks_sub["vis_info"]), len(masks_sub["vis_info"][0]["bounding_boxes"]), masks_sub["crop_images_feature"].shape, masks_sub["global_images_feature"].shape)
        
        return mask_clip, masks_sub_list
        
        
    def visualize_bounding_boxes(self, img_pil, bounding_boxes, levels, mask_id, view_id):
        """
        Visualize bounding boxes on the original image.

        Parameters:
            img_pil (PIL.Image): The original image.
            bounding_boxes (list of tuples): List of bounding box coordinates (x1, y1, x2, y2).
            levels (list of int): Corresponding levels for each bounding box.
            mask_id (int): ID of the mask.
            view_id (int): ID of the view.
        """
        img = np.array(img_pil)  # Convert PIL image to NumPy array
        img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)  # Convert to BGR for OpenCV visualization
        
        for bbox, level in zip(bounding_boxes, levels):
            x1, y1, x2, y2 = bbox
            cv2.rectangle(img, (int(x1), int(y1)), (int(x2), int(y2)), color=(0, 255, 0), thickness=2)  # Green bbox
            cv2.putText(img, f"Level {level}", (int(x1), int(y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        
        # Display the visualization
        window_name = f"View {view_id}, Mask {mask_id}"
        cv2.imshow(window_name, img)
        key = cv2.waitKey(0)
        if key == 27:  # Esc key
            print("[INFO] Esc pressed. Exiting visualization.")
            cv2.destroyAllWindows()
            raise KeyboardInterrupt  # Graceful exit from visualization if Esc is pressed
